...
function creaBarraRicerca() {
  ...
  iconaLente.html(`
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="${palette.bianco}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
function aggiornaStileBottoneCountriesTerritori() {
  ...
  const COLORE_ATTIVO = palette.bianco; // palette.bianco (sfondo attivo)
  const COLORE_INATTIVO = palette.nero; // palette.nero (sfondo inattivo)
  const BORDO_ATTIVO = '1px solid' + palette.nero; // Bordo palette.nero quando attivo
  const BORDO_INATTIVO = '1px solid' + palette.bianco; // Bordo palette.bianco quando inattivo
  
  // Bottone Countries
  if (filtroCountries === 'c') {
    // Countries attivo: Sfondo palette.bianco, bordo palette.nero
    ...
    // Countries non attivo: Sfondo palette.nero, bordo palette.bianco, opacità per l'altro filtro
  ...
  // Bottone Territories
  if (filtroCountries === 't') {
    // Territories attivo: Sfondo palette.bianco, bordo palette.nero
    ...
    // Territories non attivo: Sfondo palette.nero, bordo palette.bianco, opacità per l'altro filtro
...
function aggiornaContenutoBottoniCountriesTerritori() {
  ...
  // Se attivo è palette.nero, se inattivo è palette.bianco
  const colorC = filtroCountries === 'c' ? palette.nero : palette.bianco;
  const colorT = filtroCountries === 't' ? palette.nero : palette.bianco;
...
function aggiornaBoxInfoPaese(paeseData) {
  ...
  const COLOR_BACKGROUND = palette.nero; 
