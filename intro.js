// VARIABILI GLOBALI DI STATO E FASI (non dipendono da p5, quindi restano fuori)
let scrollAmount = 0;
...
function sketch(p) {
    ...
    p.setup = function() {
            ...
            // 2. Rendi il pulsante rotondo
            button.style('border-radius', '50%'); 

            // 3. Rimuovi lo sfondo e imposta il bordo
            button.style('background-color', 'transparent');
            button.style('border', '2px solid ' + BIANCO);

            // 4. Stile per centrare l'SVG (MODIFICATO PER CENTRATURA PERFETTA)
            button.style('display', 'flex');           
            button.style('align-items', 'center');     
            button.style('justify-content', 'center'); 
            button.style('padding', '0');             
            button.style('margin', '0');             // Aggiunto: Assicura nessun margine esterno
            button.style('line-height', '0');        // Aggiunto: Cruciale per centrare elementi SVG/Inline
    ...
    p.draw = function() {
        // Usa il colore nero definito all'inizio
        p.background(NERO);
        ...
        if (scrollAmount < PHASE_WRITE_1_END) {
            ...
            // Usa il colore BIANCO con opacità variabile
            p.fill(p.color(BIANCO), p.map(fadeOutProgress, 0, 1, 255, 0));
            p.textAlign(p.CENTER, p.CENTER);
            p.textSize(currentFontSize);
            p.text(texts[1], x, y);
            ...
            p.fill(p.color(BIANCO), finalOpacity);
            p.textAlign(p.CENTER, p.CENTER);
            p.textSize(currentFontSize);
        ...
        if (showCursor) {
            ...
            if (scrollAmount < PHASE_FADE_OUT_Q_END || (scrollAmount >= PHASE_WRITE_2_START && scrollAmount < PHASE_WRITE_2_END_TYPING)) {
                ...
                // il cursore deve lampeggiare, usa il colore BIANCO quando acceso
                if (p.frameCount % 30 < 15) {
                    p.fill(p.color(BIANCO));
                } else {
                    p.fill(p.color(NERO)); // Cursore scompare nel colore di sfondo
                }
...
// INIZIALIZZA LO SKETCH
new p5(sketch);

// GESTIONE DELL'INTERAZIONE CON LO SCROLL (resta fuori)
